...
<body class="font-sans antialiased">
    ...
    <section class="container relative z-10 px-4 mx-auto post-section">
        <div class="flex flex-row gap-8 content-area">
            <!-- Main Post Content -->
            <div class="flex-1 main-content max-w-800">
    ...
    <div class="p-6 mt-12 bg-gray-800 rounded-lg shadow-lg">
        ...
        @auth
            <form action="{{ route('comment.store', $post->id) }}" method="POST" class="mb-6">
                @csrf
                <textarea name="content" rows="3" class="w-full p-3 text-white bg-gray-700 rounded" placeholder="Add a comment..."
                    required></textarea>
                <button type="submit" class="px-4 py-2 mt-2 text-white rounded bg-cyan-500 hover:bg-cyan-600">Post
                    Comment</button>
            </form>
        @else
            <p class="mb-6 text-gray-400">Please <a href="/login" class="underline text-cyan-400">login</a> to comment.</p>
        @endauth
